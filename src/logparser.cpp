...
namespace mysql
{
    ...
    void CLogParser::dispatch_events()
    {
        ...
        do
        {
            ...
            {
                _is_connected = false;
                throw CException("Recv end packet from server,  apparent master shutdown: %s", mysql_error(&_mysql));
            }
    ...
    void CLogParser::watch ( const std::string& db_name )
    {
    ...
    void CLogParser::build_db_structure()
    {
        ...
        while ((row = mysql_fetch_row(res_db)) != NULL)
        {
            if ((it_dbs = _databases.find(row[0])) != _databases.end())
            {
                ...
                {
                    CTable* tbl = it_dbs->second.get_table(row1[0]);
                    if(NULL==tbl)
                    {
                        watch(row[0],row1[0]);
                        LOGFMT_TRACE(LOG4Z_MAIN_LOGGER_ID, "WATCH:dbname=>%s,table=>%s", row[0],row1[0]);
                        tbl = it_dbs->second.get_table(row1[0]);
                    }
                    ...
                    if (tbl)
                    {
                        ...
                        {
                            tbl->add_column(row2[0], pos++, row2[1]);
                        }
