...
#define LOG_EVENT_TYPES (ENUM_END_EVENT-1)
    ...
    public:
        ...
        bool is_supported ( uint32_t event_type )
        {
    ...
    public:
        ...
        const uint8_t* get_log_name ( size_t *len ) const
        {
            if ( len ) *len = _len;
            return _new_log;
        }

        const uint8_t* get_log_name ( ) const
        {
            return _new_log;
        }

        size_t get_log_name_len ( ) const
        {
            return _len;
        }

        uint64_t get_log_pos ( ) const
        {
            return _position;
        }
    ...
    public:
        ...
        CMemBlock* used_columns_bitmap ( )
        {
            return &_used_columns_image;
        }
        
        CMemBlock* used_columns_afterimage_bitmap ( )
        {
            return &_used_columns_after_image;
        }
    ...
    public:
        ...
        const uint8_t *_rows;
    protected:
        CMemBlock _used_columns_image;
        CMemBlock _used_columns_after_image;// for UPDATE_ROWS_EVENT only
        const uint8_t *_data; //pointer to the sequence of zero or more rows. 
        size_t _len;
        int _valid;
        ...
        VDEBUG_CHUNK (
            public:
                virtual void dump ( FILE *stream )
                {
                        CLogEvent::dump ( stream );
                        fprintf ( stream,
                        "valid: %d, rowslen %d, rowflags %d, table_id %d, ncolumns %d,", 
                        _valid,
                        ( int ) _len,
                        ( int ) _row_flags,
                        ( int ) _table_id,
                        ...
                        std::string bitmap;
                        for ( uint64_t i = 0; i < _ncolumns; ++i )
                        {
                            if ( _used_columns_image.bit_at ( i ) )
                                    bitmap += "1";
                            else
                                bitmap += "0";
                        }
                        uint32_t n_bits_len = ( _ncolumns + 7 ) / 8;
                        fprintf ( stream, "ucm %s (%d bits)", bitmap.c_str ( ), n_bits_len );

                        if ( _type == UPDATE_ROWS_EVENT )
                        {
                            std::string bitmap_ai;
                            for ( uint64_t i = 0; i < _ncolumns; ++i )
                            {
                                if ( _used_columns_image.bit_at ( i ) )
                                        bitmap_ai += "1";
                            ...
                            fprintf ( stream, ", ucam %s (%d bits)", bitmap_ai.c_str ( ), n_bits_len );
                        }
                        fprintf ( stream, "\n" );
                 }
        )
